...
`v4_unified_prompt_candidate`. The version is centralized in
...
The corpus stores 30,105 tests. Its task composition, taken from the tracked
manifest, is:

| Task family | Records |
|---|---:|
| Hidden mutation reproduction | 7,636 |
...
`data/corpus/v4_unified_prompt_candidate/manifest.json`. Do not train from raw mutation
files, legacy split files, or a different corpus directory while describing
the run as the 8,237-record V4 experiment.
...
V4 uses one prompt contract for HumanEval, MBPP, BugsInPy, and SWE-bench:
task mode, expected test format, target symbols, behavioral specification,
available execution context, code under test, task, and output constraints.
Dataset names and all reference/gold/mutation/oracle fields are excluded.
MBPP targets are derived from the outer public call in official tests so
helper definitions cannot be mistaken for the entry point.
...
`data/corpus/v4_unified_prompt_candidate/`. Verify it before any experiment:

```powershell
py -X utf8 -c "from pathlib import Path; from harness.corpus import verify_corpus; print(verify_corpus(Path('data/corpus/v4_unified_prompt_candidate'))['training_records'])"
```

The command must print `8237` and must not report a hash or split failure.
...
directory and new manifest; never edit `v4_unified_prompt_candidate` in place. Every new
...
```powershell
py scripts/preflight_sft_run.py --corpus-version v4_unified_prompt_candidate --max-pairs 800 --epochs 1 --batch-size 1 --learning-rate 0.00005 --lr-scheduler-type constant_with_warmup --repository-prompt-token-limit 1024 --repository-completion-token-limit 1024 --min-function-kill-rate 0.58
```
...
```powershell
py scripts/modal_train.py --fresh --phase sft --corpus-version v4_unified_prompt_candidate --run-name v4_unified_prompt_integration_32_seed42 --seed 42 --max-pairs 32 --sft-lr-scheduler-type constant_with_warmup --no-sft-monitor-kill-rate
```
